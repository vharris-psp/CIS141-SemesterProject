...
class ConfigHelper(Helper):
    class _default_file(Enum):
        ...
        __SWITCH_CONF_JSON__ = 'backend/api/config/conf.json'
        __SWITCH_CONF_JSON__BACKUP__ = 'backend/api/config/conf.bk'
    ...
    def get_file(file: _default_file):
        return file
    _instance = None
    _running_config: Dict[str, Any] = {}
    def _config_is_valid(self):
        if not self._running_config:
            return False
        if not self._running_config.get('devices'):
            return False
        if not self._running_config.get('default_connection_settings'):
            return False
        if not self._running_config.get('building_code_map'):
            return False
        return True
    
    def __init__(self):
        self.load_config_file()
        self.devices = self._running_config.get('devices')
        self.connection_defaults = self._running_config.get('default_connection_settings')
        super().__init__()
    ...
    def load_backup_config(cls):
        ...
        try:
            with open(cls._default_file.__SWITCH_CONF_JSON__BACKUP__, 'r') as backup_file:
                cls._running_config = load(backup_file) 
    ...
    @classmethod
    def load_config_file(cls, file: _default_file = _default_file.__SWITCH_CONF_JSON__):
        """Load config from the given file or fallback to the backup"""
        try:
            with open(file, 'r') as config_file:
                cls._running_config = load(config_file)
            ...
            raise e
    def write_config(self, file: _default_file = _default_file.__SWITCH_CONF_JSON__):
        with open(file, 'w') as config_file:
            dump(self._running_config, config_file, indent=4)  # Writing as pretty JSON
    ...
    def update_selected_switches(self, selected_switches: dict): 
        for switch in self._running_config['switch_info']:
            print(switch)
    ...
    def get_all_device_configs(self):
        ...
        device_map = self._running_config['devices']
    ...
    def get_all_devices(self, filter=None) -> dict:
        ...
        try:
            if not filter:
                devices = self._running_config['devices']
    
            else:
                devices =  self._running_config['devices'][filter]
    ...
    def wlc_list(self) -> dict:
        try:
            wlcs = self._running_config['wlc_info']
            return wlcs
        except KeyError as e:
            self.app.notify(f"Config Key Error: {e}")
    def get_wlc_connection_settings(self, wlc: str | None = None) -> dict:
        return self._running_config['wlc_info']
    def set_selected_switches(self, connections: list):
        for switch in self._running_config['switch_info']: 
            if connections[switch]:
                self._running_config['switch_info'][switch]['selected'] = True
            else: 
                self._running_config['switch_info'][switch]['selected'] = False
    def building_map(self) -> dict: 
        try:
            switches = self._running_config['building_code_map']
            return switches
        except KeyError as e:
           self.app.notify(f"Config Key Error: {e}")
    def building_ip_codes(self) -> dict:
        try:
            ip_codes = self._running_config['building_ip_codes']
            return ip_codes
        except KeyError as e:
            self.app.notify(f"Config Key Error: {e}")
